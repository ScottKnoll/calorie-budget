<section class="w-full max-w-4xl">
    <div class="mb-8">
        <flux:heading size="xl" class="mb-1">Intake Review</flux:heading>
        <flux:text>All submitted client intake responses.</flux:text>
    </div>

    @forelse ($this->users as $user)
        @php $intake = $user->intakeResponse; @endphp
        ...
        <div class="mb-8 rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="flex items-center gap-3 border-b border-zinc-200 px-6 py-4 dark:border-zinc-700">
                ...
                <div>
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ $user->email }} &middot; Submitted {{ $user->intake_completed_at->format('M j, Y') }}</p>
                </div>
            </div>
            ...
            @else
                <div class="px-6 py-4">
                    <flux:text class="text-zinc-400">No intake response on file.</flux:text>
                </div>
            @endif
    ...
    @empty
        <div class="rounded-xl border border-zinc-200 bg-white px-6 py-12 text-center dark:border-zinc-700 dark:bg-zinc-900">
            <flux:heading size="lg" class="mb-1">No responses yet</flux:heading>
            <flux:text>No clients have completed the intake form.</flux:text>
        </div>
    @endforelse
</section>
